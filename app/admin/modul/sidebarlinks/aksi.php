...
if ($act == 'update') {
    ...
    if (!empty($lokasi_file)) {
        if (($ukuran == 0) or ($ukuran == 02) or ($ukuran > 2060817)) {
            ...
            $edit = $db->connection("SELECT gambar FROM sidebarlinks WHERE id_link='$_POST[id_link]'");
            $tedit = $edit->fetch(PDO::FETCH_ASSOC);

            unlink("images/sidebarlinks/$tedit[gambar]");
            unlink("images/sidebarlinks/small/$tedit[gambar]");

            $res = lopoUpload($seojdul . '-' . $acak, 'sidebarlinks');
            ...
            if ($res == true) {
                try {
                    ...
                    $db->update("sidebarlinks", $datas, "id_link= '$_POST[id_link]' ");

                    $pathToImage = 'images/sidebarlinks/' . $nama_file_unik;
                    $pathSmall = 'images/sidebarlinks/small/' . $nama_file_unik;
                    lopoCompress('sidebarlinks', $pathToImage, $tipe_file2, 1);
                    lopoCompress('sidebarlinks/small', $pathToImage, $tipe_file2, 6);
        ...
        try {
            ...
            $db->update("sidebarlinks", $datas, "id_link= '$_POST[id_link]' ");
            $msg->info('Sidebar Links Berhasil diedit');
            echo "<script>alert('Sidebar Links Berhasil diedit'); window.location = '$hal-edit-$_POST[id_link]'</script>";
        } catch (PDOException $e) {
            echo "<script>alert('Sidebar Links Gagal diedit!'); window.location = '$hal-edit-$_POST[id_link]'</script>";
        }
    }
        ...
        if (($ukuran == 0) or ($ukuran == 02) or ($ukuran > 2060817)) {
            ...
            if ($res == true) {
                try {
                    ...
                    $saved = $db->insert('sidebarlinks', $datas);
                    $insertId = $db->lastId();
...
// remove modul
elseif ($act == 'remove') {
    $edit = $db->connection("SELECT gambar FROM sidebarlinks WHERE id_link=$id ");
    $rr = $edit->fetch(PDO::FETCH_ASSOC);
    unlink("images/sidebarlinks/$rr[gambar]");
    unlink("images/sidebarlinks/small/$rr[gambar]");

    $del = $db->delete("sidebarlinks", "id_link=$id ");
