...
use \Gumlet\ImageResize;
    ...
    if ( $act == 'update' ) {
        ...
        if ( !empty( $nama_file ) ) {
            if ( ( $ukuran == 0 ) OR ( $ukuran == 02 ) OR ( $ukuran>9060817 ) ) {
                ...
                $edit = $db->connection( "SELECT gambar FROM kendaraan2 WHERE id_kendaraan='$_POST[id_kendaraan]'" );
                $tedit = $edit->fetch( PDO::FETCH_ASSOC );
                unlink( "images/kendaraan2/$tedit[gambar]" );
                unlink( "images/kendaraan2/small/$tedit[gambar]" );

                // Upload Image
                $res = lopoUpload( $judul_seo.'-'.$acak, 'kendaraan2' );
                if ( $res == true ) {
                    try {
                        ...
                        $saved = $db->update( 'kendaraan2', $datas, "id_kendaraan = '$_POST[id_kendaraan]' " );
                        $pathToImage = 'images/kendaraan2/'.$nama_file_unik;
                        $pathSmall   =  'images/kendaraan2/small/'.$nama_file_unik;
                        lopoCompress( 'kendaraan2', $pathToImage, $tipe_file2, 1 );
                        lopoCompress( 'kendaraan2/small', $pathToImage, $tipe_file2, 6 );
    ...
    elseif (  $act == 'add' ) {
        ...
        if ( $res == false ) {
            ...
            try {
                ...
                $saved = $db->insert( 'kendaraan2', $datas );
                $insertId = $db->lastId();
                ...
                lopoCompress( 'kendaraan2', $pathToImage, $tipe_file2, 1 );
                lopoCompress( 'kendaraan2/small', $pathToImage, $tipe_file2, 4 );
    ...
    // remove modul
    elseif ( $act == 'remove' ) {
        $edit = $db->connection( "SELECT gambar FROM kendaraan2 WHERE id_kendaraan='$id'" );
        $rr = $edit->fetch( PDO::FETCH_ASSOC );
        unlink( "images/kendaraan2/$rr[gambar]" );
        unlink( "images/kendaraan2/small/$rr[gambar]" );

        $del = $db->delete( 'kendaraan2', "id_kendaraan='$id'" );

        $msg->success('Data berhasil dihapus');
        echo "<script>window.location = 'kendaraan2'</script>";
    }
