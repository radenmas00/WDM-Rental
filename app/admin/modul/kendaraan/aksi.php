...
use \Gumlet\ImageResize;
    ...
    if ( $act == 'update' ) {
        ...
        if ( !empty( $nama_file ) ) {
            if ( ( $ukuran == 0 ) OR ( $ukuran == 02 ) OR ( $ukuran>9060817 ) ) {
                ...
                $edit = $db->connection( "SELECT gambar FROM kendaraan WHERE id_kendaraan='$_POST[id_kendaraan]'" );
                $tedit = $edit->fetch( PDO::FETCH_ASSOC );
                unlink( "images/kendaraan/$tedit[gambar]" );
                unlink( "images/kendaraan/small/$tedit[gambar]" );

                // Upload Image
                $res = lopoUpload( $judul_seo.'-'.$acak, 'kendaraan' );
                if ( $res == true ) {
                    try {
                        ...
                        $saved = $db->update( 'kendaraan', $datas, "id_kendaraan = '$_POST[id_kendaraan]' " );
                        $pathToImage = 'images/kendaraan/'.$nama_file_unik;
                        $pathSmall   =  'images/kendaraan/small/'.$nama_file_unik;
                        lopoCompress( 'kendaraan', $pathToImage, $tipe_file2, 1 );
                        lopoCompress( 'kendaraan/small', $pathToImage, $tipe_file2, 6 );
    ...
    elseif (  $act == 'add' ) {
        ...
        if ( $res == false ) {
            ...
            try {
                ...
                $saved = $db->insert( 'kendaraan', $datas );
                $insertId = $db->lastId();
                ...
                lopoCompress( 'kendaraan', $pathToImage, $tipe_file2, 1 );
                lopoCompress( 'kendaraan/small', $pathToImage, $tipe_file2, 4 );
    ...
    // remove modul
    elseif ( $act == 'remove' ) {
        $edit = $db->connection( "SELECT gambar FROM kendaraan WHERE id_kendaraan='$id'" );
        $rr = $edit->fetch( PDO::FETCH_ASSOC );
        unlink( "images/kendaraan/$rr[gambar]" );
        unlink( "images/kendaraan/small/$rr[gambar]" );

        $del = $db->delete( 'kendaraan', "id_kendaraan='$id'" );

        $msg->success('Data berhasil dihapus');
        echo "<script>window.location = 'kendaraan'</script>";
    }
